...
**GA4 + Meta Pixel** (`services/analytics.ts`, consent-gated). Set `VITE_GA_MEASUREMENT_ID` and/or `VITE_META_PIXEL_ID`. Scripts load only after the visitor accepts the cookie banner. Events: `page_view` on every route, `lead` (popup), `begin_checkout`, `purchase`. The server CSP already allowlists the GA/Meta origins. (Optional later: Meta Conversions API for server-side, ad-blocker-proof purchase tracking.)
